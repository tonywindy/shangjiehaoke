 ...
 * 默认导出金句数据
 */
export default educationQuotes;

/**
 * 根据分类获取金句
 * @param {string} category - 金句分类
 * @returns {Array} 指定分类的金句数组
 */
export const getQuotesByCategory = (category) => {
  return educationQuotes.filter(quote => quote.category === category);
};

/**
 * 根据标签获取金句
 * @param {string} tag - 金句标签
 * @returns {Array} 包含指定标签的金句数组
 */
export const getQuotesByTag = (tag) => {
  return educationQuotes.filter(quote => quote.tags.includes(tag));
};

/**
 * 随机获取指定数量的金句
 * @param {number} count - 需要获取的金句数量
 * @returns {Array} 随机金句数组
 */
export const getRandomQuotes = (count = 1) => {
  const shuffled = [...educationQuotes].sort(() => 0.5 - Math.random());
